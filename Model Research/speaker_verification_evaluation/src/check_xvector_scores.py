...
def build_speaker_events(
    ...
    for row in utterances.itertuples(index=False):
        ...
        if audio_path.exists():
            speaker_events[str(row.speaker_id)][
                str(row.event_id)
            ].append(audio_path)
...
def create_same_speaker_pairs(
    ...
    eligible_speakers = [
        speaker_id
        for speaker_id, events in speaker_events.items()
        if len(events) >= 2
    ]
    ...
    # First include different speakers across the dataset.
    ...
    for speaker_id in shuffled_speakers:
        ...
        pair_key = tuple(
            sorted((str(audio_a), str(audio_b)))
        )
...
def create_different_speaker_pairs(
    speaker_events: dict,
    target_count: int,
    rng: random.Random,
) -> list[dict]:
    ...
    while (
        ...
        speaker_a, speaker_b = rng.sample(
            speakers,
            2,
        )

        event_a = rng.choice(
            list(speaker_events[speaker_a].keys())
        )
        event_b = rng.choice(
            list(speaker_events[speaker_b].keys())
        )

        audio_a = rng.choice(
            speaker_events[speaker_a][event_a]
        )
        audio_b = rng.choice(
            speaker_events[speaker_b][event_b]
        )
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Check x-vector cosine scores using "
            "multiple speakers."
        )
    )
    ...
    parser.add_argument(
        ...
        default=Path(
            "outputs/diagnostics/"
            "xvector_multi_person_scores.csv"
        ),
    )
    ...
    speaker_events = build_speaker_events(
        utterances,
        args.audio_root,
    )

    print(
        f"Speakers with available audio: "
        f"{len(speaker_events)}"
    )

    same_pairs = create_same_speaker_pairs(
        speaker_events,
        args.same_pairs,
        rng,
    )

    different_pairs = create_different_speaker_pairs(
        speaker_events,
        args.different_pairs,
        rng,
    )
    ...
    def get_embedding(audio_path: Path):
        ...
        if path_key not in embedding_cache:
            embedding_cache[path_key] = (
                adapter.extract_embedding(
                    audio_path
                )
            )
    ...
    for pair_id, pair in enumerate(
        ...
        similarity = adapter.calculate_similarity(
            embedding_a,
            embedding_b,
        )
        ...
        results.append(
            {
                ...
                "audio_a": str(pair["audio_a"]),
                "audio_b": str(pair["audio_b"]),
                "label": pair["label"],
    ...
    same_scores = results_df.loc[
        results_df["label"] == 1,
        "similarity",
    ].to_numpy()

    different_scores = results_df.loc[
        results_df["label"] == 0,
        "similarity",
    ].to_numpy()
    ...
    print(
        f"Median: "
        f"{np.median(different_scores):.4f}"
    )
    print(
        f"Minimum:{different_scores.min():.4f}"
    )
    print(
        f"Maximum:{different_scores.max():.4f}"
    )

    print("\nOverall diagnostic")
    print(f"ROC-AUC:             {roc_auc:.4f}")
    print(
        f"Temporary threshold: "
        f"{best_threshold:.4f}"
    )
    print(
        f"Temporary accuracy:  "
        f"{best_accuracy:.4f}"
    )
    print(
        f"Unique audio files:  "
    ...
    if roc_auc >= 0.90:
        print(
            "The model separates the speakers well "
            "on these sampled pairs."
        )
    elif roc_auc >= 0.70:
        print(
            "The model has some separation, but the "
            "score distributions overlap."
        )
    else:
        print(
            "The raw cosine scores provide weak "
            "speaker separation on this dataset."
        )
